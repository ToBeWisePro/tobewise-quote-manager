...
export default function AddQuotePage() {
  ...
  const [authorDebounceTimer, setAuthorDebounceTimer] =
    useState<NodeJS.Timeout | null>(null);
  const [existingQuotes, setExistingQuotes] = useState<
    Array<{ id: string; quoteText: string; author: string }>
  >([]);
  const [similarQuote, setSimilarQuote] = useState<{
    quote: { id: string; quoteText: string; author: string };
    similarity: number;
  } | null>(null);
  const [autoGeneration, setAutoGeneration] = useState(true);
  const [generationTimeout, setGenerationTimeout] =
    useState<NodeJS.Timeout | null>(null);
    ...
    const fetchExistingQuotes = async () => {
      try {
        const querySnapshot = await getDocs(
          collection(getFirestoreDb(), "quotes"),
        );
  ...
  const handleDelayedGeneration = useCallback(
    async (quoteText: string) => {
      ...
      if (!useGeminiAutofill) {
        console.log(
          "[DEBUG] Skipping AI generation: useGeminiAutofill is false",
        );
        return;
      }
      ...
      try {
        ...
        if (
          generatedAuthor &&
          !generatedAuthor.toLowerCase().includes("unknown")
        ) {
  ...
  const handleQuoteBlur = useCallback(async () => {
    ...
    try {
      ...
      const stored =
        typeof window !== "undefined" ? localStorage.getItem("subjects") : null;
      const allSubjects = stored ? (JSON.parse(stored) as string[]) : [];
  ...
  const regenerateLinks = useCallback(
  ...
  useEffect(() => {
    ...
    if (newQuote.author.trim().toLowerCase() === lastLinkedAuthor.toLowerCase())
      return;
    ...
    try {
      ...
      try {
        ...
        const updated = Array.from(
          new Set([
            ...existing,
            ...newQuote.subjects.map((s) => s.trim().toLowerCase()),
          ]),
        );
  ...
  const hasQuoteChanges = useMemo(
    () =>
      Boolean(
        newQuote.quoteText.trim() ||
        newQuote.author.trim() ||
        newQuote.subjects.some((subject) => subject.trim()) ||
        (newQuote.authorLink ?? "").trim() ||
        (newQuote.contributedBy ?? "").trim() ||
        (newQuote.videoLink ?? "").trim(),
      ),
    [newQuote],
  );

  const canSaveQuote = hasQuoteChanges && similarityLevel !== "identical";
  ...
  return (
    <DashboardPageShell contentClassName="mx-auto flex w-full max-w-4xl min-w-0 flex-col gap-6">
      <DashboardPageHeader
                  ...
                  <div className="ml-3">
                    ...
                    <div className="mt-1 text-sm text-red-700">
                      <p className="font-medium">
                        &ldquo;{similarQuote?.quote.quoteText}&rdquo;
                      </p>
                ...
                <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 text-xs text-white bg-gray-900 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap z-10">
                  <div className="relative">
                    <div className="text-center">
                      <strong>Auto-run AI after typing:</strong>
                      <br />
                      Automatically run AI to fill metadata
                      <br />
                      1 second after you stop typing.
                      <br />
                      When off, AI only runs when you click out
                      <br />
            ...
            <div className="mt-2 flex items-center">
              ...
              <div className="relative ml-2 group">
                ...
                <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 text-xs text-white bg-gray-900 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap z-10">
                  <div className="relative">
                    <div className="text-center">
                      <strong>Enable AI autofill:</strong>
                      <br />
                      Allow Gemini AI to fill author, subjects,
                      <br />
                      and links. If off, you must enter all fields
                      <br />
                ...
                <div className="flex items-start">
                  ...
                  <div className="ml-3">
                    <h3 className="text-sm font-medium text-yellow-800">
                      Similar quote detected (
                      {Math.round(similarQuote.similarity * 100)}% match)
                    </h3>
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                onChange={(e) =>
                  setNewQuote({ ...newQuote, authorLink: e.target.value })
                }
